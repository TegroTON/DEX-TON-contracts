...
export const DexContextProvider: React.FC<Props> = ({ children }) => {
    ...
    const updatePair = async ({
        ...
        let {
            leftToken, rightToken, rightBalance, leftBalance, leftWallet, rightWallet,
        } = newXXXPair;
        ...
        const { leftReserved, rightReserved } = getPairByTokens(pairs, leftToken, rightToken);

        console.log('left', leftReserved.toString(), 'right', rightReserved.toString());

        newXXXPair = {
            ...newXXXPair, leftWallet, rightWallet, leftBalance, rightBalance, leftReserved, rightReserved,
        };
    ...
    const updateSlippage = (newSlippage: number) => {
        ...
        const newSwapParams = { ...swapParams, slippage: correctSlippage };
    ...
    useEffect(() => {
        updatePair({ when: 'swap' }).then();
        updatePair({ when: 'pool' }).then();
        updatePoolPositions().then();
