...
const normalizePair = async (p: any, tokens: Tokens): Promise<Pair> => ({
    ...
    leftReserved: new Coins(p.leftReserved, { isNano: true, decimals: getDecimals(tokens, new Address(p.leftAddress)) }),
    rightReserved: new Coins(p.rightReserved, { isNano: true, decimals: getDecimals(tokens, new Address(p.rightAddress)) }),
});
