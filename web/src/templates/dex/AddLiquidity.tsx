...
import { getOutAmount, getStakeAmount } from '../../ton/dex/utils';
...
export default function AddLiquidityPage() {
    ...
    const {
        poolPair, poolParams, updatePoolParams, walletInfo, tokens,
    } = useContext(DexContext) as DexContextType;

    let { slippage, inAmount, outAmount } = poolParams;
    const {
    ...
    const updateAmount = (side: ('left' | 'right')) => {
        ...
        if (value) {
            ...
            if (side === 'left') {
                ...
                updatePoolParams({ ...poolParams, inAmount, outAmount });
                setValue('right', outAmount.toString());
            } else {
                outAmount = new Coins(inAmount).div(leftPrice.toString());
                updatePoolParams({ ...poolParams, outAmount: inAmount, inAmount: outAmount });
                        ...
                        <form className="card-body p-40" action="">
                            ...
                            <div className="input-group mb-4">
                                <input
                                    ...
                                    // disabled
                                    {...register('right', {
                                        onChange: (event) => { fieldNormalizer('right', event.target.value, setValue); updateAmount('right'); },
                                        validate: (value) => value && parseFloat(value) > 0,
                                    })}
                                />
                                <div className="input-group-text p-1">
                            ...
                            <div className="card-alert p-4 bg-soft-blue rounded-8">
                                <ul className="list-unstyled">
                                    <li className="list-item d-flex mb-4">
                                        <span className="me-auto fw-500">BTCB per ALPACA</span>
                                        <span
                                    ...
                                    <li className="list-item d-flex mb-4">
                                        <span className="me-auto fw-500">ALPACA per BTCB</span>
                                        <span
                                            className="text-muted"
                                        >
                                            93071.5
