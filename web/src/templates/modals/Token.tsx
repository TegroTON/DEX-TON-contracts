...
export function TokenModal(props: { side: 'Left' | 'Right' }) {
    ...
    const {
        walletInfo, updatePair, swapPair, tokens, pairs,
    } = useContext(DexContext) as DexContextType;

    let { leftToken: mySide, rightToken: otherSide } = swapPair;
    ...
    const changeSelected = async (tokenAddr: string) => {
        ...
        if (!mySide.eq(tokenAddress)) {
            if (side === 'Left') {
                await updatePair({when: "swap", newPair: getPairByTokens(pairs, otherSide, mySide)});
            } else {
                await updatePair({when: "swap", newPair: getPairByTokens(pairs, mySide, otherSide)});
            }
        }
    };
