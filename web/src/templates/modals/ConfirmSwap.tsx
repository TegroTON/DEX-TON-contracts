...
export function ConfirmSwapModal() {
    const {
        walletInfo, swapPair, swapParams, tokens, updatePair,
    } = useContext(DexContext) as DexContextType;
    const { slippage, inAmount, outAmount } = swapParams;
    ...
    const handleConfirm = async () => {
        ...
        if (swapPair.rightToken.eq(TON_ADDRESS)) {
            ...
            await walletInfo?.sendTransaction({
                ...
                value: new Coins(0.15).toNano(),
                payload: BOC.toBase64Standard(payload)
                    // .replaceAll('+', '-')
                    // .replaceAll('/', '_'),
            });
        } else {
            const payload = DexBetaPairContract.createTonSwapRequest(minReceived);
            await walletInfo?.sendTransaction({
                to: dexPair.address.toString(),
                value: new Coins(inAmount).add(new Coins(0.15)).toNano(),
                payload: BOC.toBase64Standard(payload)
                    // .replaceAll('+', '-')
                    // .replaceAll('/', '_'),
            });
